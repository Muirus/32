module.exports = async (bot, message, args, argsF) => {
    const channelId = '1086596936875900968'; // Замените на фактический ID канала, где находится сообщение
    const messageId = '1106308109984596029'; // Замените на фактический ID сообщения, которое нужно отредактировать
  
    // Получаем канал и сообщение
    const channel = await bot.channels.fetch(channelId);
    const targetMessage = await channel.messages.fetch(messageId);
  
    // Создаем новое содержимое сообщения
    const newContent = {
        embeds: [
            {
                title: "```Соглашение:```",
